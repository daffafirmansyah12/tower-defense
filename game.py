...
from config import *
from config import to_virtual_pos, flip_to_screen
...
class IceTower(Tower):
    ...
    def shoot(self, enemies):
        ...
        if target:
            ...
            bullets.append(b)
            self._cooldown = 40
            SoundManager.instance().play_shoot_ice()
...
class FireTower(Tower):
    ...
    def shoot(self, enemies):
        ...
        if target1:
            ...
            self._cooldown = 35
            SoundManager.instance().play_shoot_fire()
...
class LightningTower(Tower):
    def __init__(self, x, y):
        super().__init__(x, y, LIGHT_CYAN, 1, 100)
        self._base_cost = 80
    ...
    def shoot(self, enemies):
        ...
        if in_range_enemies:
            ...
            self._cooldown = 50
            SoundManager.instance().play_shoot_lightning()
...
class LaserTower(Tower):
    def __init__(self, x, y):
        super().__init__(x, y, MAGENTA, 4, 100)
        self._base_cost = 100
    ...
    def shoot(self, enemies):
        ...
        if target:
            ...
            self._cooldown = 30
            SoundManager.instance().play_shoot_laser()
...
class Wave:
    ...
    def _wave_scale(self, base, exponent=0.18):
        """Scaling eksponensial jumlah musuh per wave.
        Wave 1 = base, wave 10 = base * ~5.2, wave 20 = base * ~27.
        Pakai exponent lebih kecil agar tidak terlalu meledak.
        """
        return max(0, int(base * (1.0 + exponent) ** (self.wave_number - 1)))

    def _generate_enemies(self):
        w = self.wave_number

        # Jumlah musuh scale eksponensial (exp 0.12) — lebih halus
        fast_count   = self._wave_scale(3 + w // 2,   exponent=0.06)
        tank_count   = self._wave_scale(1 + w // 3,   exponent=0.06)
        slow_count   = self._wave_scale(max(0, w // 4), exponent=0.06)
        flying_count = 0 if w < 3 else self._wave_scale((w - 2) // 2, exponent=0.06)

        boss_count = 1 if (w % 5 == 0 and w > 0) else 0

        shield_count  = 0 if w < 5 else self._wave_scale((w - 4) // 2,  exponent=0.06)
        healing_count = 0 if w < 6 else self._wave_scale((w - 5) // 3,  exponent=0.06)
        split_count   = 0 if w < 7 else self._wave_scale((w - 6) // 2,  exponent=0.06)
    ...
    def update(self):
        ...
        if self.spawn_timer >= self.spawn_rate:
            if self.current_spawn_index < len(self.enemies_to_spawn):
                ...
                # Wave health scaling: +15% HP per wave (eksponensial)
                wave_hp_mult = 1.15 ** (self.wave_number - 1)
                scaled_config = dict(self.difficulty_config)
                scaled_config['enemy_health_multiplier'] = (
                    self.difficulty_config['enemy_health_multiplier'] * wave_hp_mult
                )
                new_enemy = enemy_class(-50, random.randint(100, 500), scaled_config)
...
class Game:
    def __init__(self, difficulty=Difficulty.NORMAL):
        ...
        self.pending_tower_type = None   # tower yang sedang "dipegang" untuk ditempatkan
    ...
    def is_valid_tower_placement(self, x, y):
        # Area valid: hindari HUD atas (88px), HUD bawah (54px), dan tepi kiri/kanan
        min_x = 30
        max_x = WIDTH - 30
        min_y = 95          # tepat di bawah top HUD
        max_y = HEIGHT - 60  # tepat di atas bottom HUD

        if not (min_x <= x <= max_x and min_y <= y <= max_y):
            return False

        # Cek tidak tumpang tindih dengan tower yang sudah ada (radius 40px)
        for tower in self.towers:
            if math.hypot(x - tower._x, y - tower._y) < 40:
                return False
    ...
    def update(self):
        ...
        SoundManager.instance().update()  # update cooldown shoot
    ...
    def _complete_wave(self):
        ...
        self.gap_timer = 0
        SoundManager.instance().play_wave_complete()
    ...
    def place_tower(self, tower_type, x=None, y=None):
        if len(self.towers) >= self.MAX_TOWERS:
            self.sell_feedback_text = f"Max towers reached! ({self.MAX_TOWERS} limit)"
            self.sell_feedback_timer = 90
            self.pending_tower_type = None
            return

        if x is None or y is None:
            x, y = to_virtual_pos(pygame.mouse.get_pos())

        if not self.is_valid_tower_placement(x, y):
            return   # tetap dalam pending mode agar user bisa coba posisi lain

        costs = {"ice": 40, "fire": 60, "lightning": 80, "laser": 100}
        classes = {
            "ice":       IceTower,
            "fire":      FireTower,
        ...
        cost = costs.get(tower_type, 999)
        if self.money < cost:
            self.pending_tower_type = None
            return

        cls = classes.get(tower_type)
        if cls:
            self.towers.append(cls(x, y))
            self.money -= cost
            self.selected_tower = None
            self.towers_placed += 1
            self.pending_tower_type = None   # placement selesai
    ...
    def upgrade_tower(self):
        if not self.selected_tower:
            return
        if self.selected_tower.level >= self.MAX_LEVEL:
            self.sell_feedback_text = "Tower maxed out! (Level 3 max)"
            self.sell_feedback_timer = 90
            return
        if self.money >= 30:
    ...
    def save_game(self) -> bool:
        """Simpan state game saat ini. Return True jika berhasil."""
        from save_manager import SaveManager
        return SaveManager.save(self)
        ...
        while self.running:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    mouse_x, mouse_y = to_virtual_pos(pygame.mouse.get_pos())

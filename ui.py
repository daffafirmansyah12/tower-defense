...
import os
from config import *
from config import to_virtual_pos
...
class Button:
    ...
    Styles:
      'default'  → btn assets (normal/hover/pressed)
      'success'  → btn assets + green tint overlay
      'danger'   → btn assets + red tint overlay
      'ghost'    → btn assets dengan alpha rendah (disabled look tapi tetap klikable)
      'small'    → sama seperti default tapi font lebih kecil
      'disabled' → btn_disabled assets, tidak bisa diklik
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            if self.rect.collidepoint(event.pos):
                self.pressed = True
                SoundManager.instance().play_button_click()
                return True
    ...
    def draw(self, surface):
        ...
        # 2. Gambar base (assets atau fallback rect)
        if base_img:
            surface.blit(base_img, self.rect.topleft)
        else:
            # Fallback tanpa assets
            fb_colors = {
        ...
        # 3. Style color-tint overlay di atas assets (hanya untuk danger/success/ghost)
        TINT_MAP = {
            'danger':  (180, 30,  30,  55),
...
class MainMenu:
    ...
    def __init__(self):
        ...
        self.t = 0.0
        self.choice = None     # 'new_game' | 'load_game' | 'settings' | 'quit'
        ...
        self._save_info = SaveManager.get_save_info()   # None jika tidak ada
        has_save = self._save_info is not None
        ...
        self.buttons = [
            ...
            Button((cx-bw//2, by+gap,    bw, bh), 'LOAD GAME',  'default', shortcut='L',
                   disabled=not has_save),
            Button((cx-bw//2, by+gap*2,  bw, bh), 'SETTINGS',  'default', shortcut='S'),
            Button((cx-bw//2, by+gap*3,  bw, bh), 'EXIT',      'danger',  shortcut='ESC'),
        ]
        self._actions = ['new_game', 'load_game', 'settings', 'quit']
    ...
    def update(self):
        self.t += 0.016
        mp = to_virtual_pos(pygame.mouse.get_pos())
...
class PauseMenu:
    ...
    def __init__(self):
        self.choice = None  # 'resume' | 'settings' | 'main_menu'
        ...
        self.buttons = [
            ...
            Button((cx-bw//2, by+gap,   bw, bh), 'SAVE GAME',   'default', shortcut='F5'),
            Button((cx-bw//2, by+gap*2, bw, bh), 'SETTINGS',    'default', shortcut='S'),
            Button((cx-bw//2, by+gap*3, bw, bh), 'MAIN MENU',   'danger',  shortcut='M'),
        ]
        self._actions = ['resume', 'save_game', 'settings', 'main_menu']
        self._key_map = {
            pygame.K_ESCAPE: 'resume',
            pygame.K_F5:     'save_game',
            pygame.K_s:      'settings',
            pygame.K_m:      'main_menu',
        }
        self.save_feedback_timer = 0   # berapa frame teks konfirmasi save ditampilkan
        self.save_feedback_ok    = True
    ...
    def draw(self, surface):
        ...
        # Panel tengah — diperbesar karena ada 4 tombol
        pw, ph = 320, 440
        ...
        # Feedback save (muncul beberapa detik setelah save)
        if self.save_feedback_timer > 0:
            self.save_feedback_timer -= 1
            alpha = int(255 * min(1.0, self.save_feedback_timer / 30))
            msg   = 'Game Saved!' if self.save_feedback_ok else 'Save Failed!'
            color = THEME['success'] if self.save_feedback_ok else THEME['danger']
            fb = Fonts.small.render(msg, True, color)
            fb.set_alpha(alpha)
            surface.blit(fb, (WIDTH//2 - fb.get_width()//2, panel_rect.bottom + 10))
...
class InGameHUD:
    ...
    def draw_placement_preview(self, surface, tower_type: str, mx: int, my: int, valid: bool):
        """
        Gambar ghost tower di posisi cursor saat sedang dalam placement mode.
        Hijau = valid, Merah = tidak valid.
        """
        TOWER_INFO = {
            'ice':       {'color': BLUE,       'range': 100, 'label': 'Ice Tower   $40'},
            'fire':      {'color': RED,         'range': 100, 'label': 'Fire Tower  $60'},
            'lightning': {'color': LIGHT_CYAN,  'range': 100, 'label': 'Lightning   $80'},
            'laser':     {'color': MAGENTA,     'range': 100, 'label': 'Laser Tower $100'},
        }
        info = TOWER_INFO.get(tower_type, {'color': WHITE, 'range': 100, 'label': tower_type})

        border_col = THEME['success'] if valid else THEME['danger']
        ghost_col  = (*info['color'], 120)

        # Range circle
        range_surf = pygame.Surface((surface.get_width(), surface.get_height()), pygame.SRCALPHA)
        pygame.draw.circle(range_surf, (*border_col, 25), (mx, my), info['range'])
        pygame.draw.circle(range_surf, (*border_col, 80), (mx, my), info['range'], 1)
        surface.blit(range_surf, (0, 0))

        # Ghost tower body
        ghost = pygame.Surface((40, 40), pygame.SRCALPHA)
        pygame.draw.circle(ghost, ghost_col, (20, 20), 18)
        pygame.draw.circle(ghost, (*border_col, 200), (20, 20), 18, 2)
        surface.blit(ghost, (mx - 20, my - 20))

        # Label di bawah cursor
        label = info['label']
        if not valid:
            label += '  ✗'
        lc = THEME['success'] if valid else THEME['danger']
        ls = Fonts.small.render(label, True, lc)
        # Pastikan teks tidak keluar layar
        tx = min(mx + 15, surface.get_width() - ls.get_width() - 4)
        ty = min(my + 18, surface.get_height() - 20)
    ...
    def draw_top_bar(self, surface, game_state_data: dict):
        ...
        tower_c = CYAN if towers < 8 else YELLOW if towers < self.MAX_TOWERS else RED
    ...
    def draw_bottom_bar(self, surface, game_state_data: dict, wave_info: dict = None):
        ...
        pending   = game_state_data.get('pending_tower_type')

        if pending:
            # Mode placement aktif: tampilkan info tower yang sedang dipegang
            TOWER_LABELS = {
                'ice':       ('Ice Tower',       40,  BLUE),
                'fire':      ('Fire Tower',      60,  RED),
                'lightning': ('Lightning Tower', 80,  LIGHT_CYAN),
                'laser':     ('Laser Tower',     100, MAGENTA),
            }
            name, cost, col = TOWER_LABELS.get(pending, (pending, '?', WHITE))
            pulse = abs(math.sin(time.time() * 4))
            pc = lerp_color(col, WHITE, pulse * 0.4)
            line1 = Fonts.small.render(
                f'📍 Placing: {name}  (${cost})   Klik kiri = taruh   RC / ESC = batal',
                True, pc
            )
            line2 = Fonts.tiny.render(
                '1=Ice  2=Fire  3=Lightning  4=Laser  (tekan angka lagi untuk ganti)',
                True, THEME['muted']
            )
            ...
            is_maxed = sel_tower.get('level', 1) >= 3
            maxed_tag = '  ★ MAXED OUT' if is_maxed else ''
            upgrade_hint = '  [U]=Max!' if is_maxed else '  [U]=Upgrade($30)'
            line1 = Fonts.small.render(
                f"{sel_tower['type']}  Lv.{sel_tower['level']}{maxed_tag}  DMG:{sel_tower['damage']}",
                True, (255, 200, 0) if is_maxed else YELLOW
            )
            line2 = Fonts.tiny.render(
                f"Target: {sel_tower['mode']}   Sell: ${sel_tower['sell_value']}"
                f"{upgrade_hint}  [T]=Target  [RC]=Sell",
                True, (255, 160, 0) if is_maxed else THEME['info']
            )
            surface.blit(line1, (RIGHT_X, BAR_Y + 6))
            surface.blit(line2, (RIGHT_X, BAR_Y + 28))
        else:
            ...
            hint1 = Fonts.tiny.render(
                '1=Ice($40)  2=Fire($60)  3=Lightning($80)  4=Laser($100)  → tekan lalu klik',
                True, THEME['muted']
            )
            hint2 = Fonts.tiny.render(
                'Klik kiri=pilih tower  RC=jual  U=upgrade  T=target  Z=turbo  ESC=pause',
                True, THEME['muted']
            )
    ...
    def draw_wave_complete_banner(self, surface, wave_num: int, bonus: int, time_left: float):
        ...
        s1 = Fonts.heading.render(msg1, True, THEME['success'])
        s1.set_alpha(alpha)
        surface.blit(s1, s1.get_rect(center=(WIDTH//2, pr.y + 35)))
        s2 = Fonts.body.render(msg2, True, THEME['accent'])
        s2.set_alpha(alpha)
        surface.blit(s2, s2.get_rect(center=(WIDTH//2, pr.y + 72)))
        s3 = Fonts.small.render(msg3, True, THEME['muted'])
        s3.set_alpha(alpha)
    ...
    def draw_sell_feedback(self, surface, text: str, alpha_ratio: float):
        ...
        # Pilih warna teks: oranye untuk "maxed out", hijau untuk sold/lainnya
        color = THEME['warning'] if 'maxed' in text.lower() or 'max' in text.lower() else THEME['success']
        s = Fonts.heading.render(text, True, color)
        s.set_alpha(a)
        # Melayang ke atas
    ...
    def draw_mode_change_feedback(self, surface, text: str, alpha_ratio: float):
        a = int(255 * alpha_ratio)
        s = Fonts.body.render(text, True, THEME['info'])
        s.set_alpha(a)
...
class DifficultySelectMenu:
    ...
    def __init__(self):
        ...
        configs = [
            {
                'label': 'EASY',
                'desc':  ['Enemies 30% weaker & slower', 'Starting money: $200', 'Base HP: 20', 'Tower range: 100'],
                'color': THEME['success'],
                'diff':  'EASY',
            },
            {
                'label': 'NORMAL',
                'desc':  ['Balanced challenge', 'Starting money: $150', 'Base HP: 10', 'Tower range: 100'],
                'color': THEME['info'],
                'diff':  'NORMAL',
            },
            {
                'label': 'HARD',
                'desc':  ['Enemies 50% stronger', 'Starting money: $100', 'Base HP: 5', 'Tower range: 100'],
                'color': THEME['danger'],
                'diff':  'HARD',
            },
        ]
